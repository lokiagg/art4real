...
//    save_latency(++ count);
    ...
    if (dsm->getMyNodeID() == 0) 
    {
      ...
      pritnf("write cnt: %" PRIu64",write time: %" PRIu64",write avg time : %lf ,cas cnt: %" PRIu64",cas time: %" PRIu64" ,cas avg time %lf \n",write_cnt_total,write_time_total,(double)write_time_total *1.0/write_cnt,cas_cnt_total,cas_time_total,(double)cas_time_total *1.0/cas_cnt_total);
    } 
/*
    if (dsm->getMyNodeID() == 0) {
