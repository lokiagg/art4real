...
thresholds                 = fig_params['threshold']
...
def main(cmd: CMDManager, tp: LogParser):
    ...
    for method in methods:
        project_dir = f"{home_dir}/art4real"
        work_dir = f"{project_dir}/build"
        env_cmd = f"cd {work_dir}"

        for threshold in thresholds:
            # change config
            sed_cmd = generate_sed_cmd('./include/Common.h', method == 'Sherman', 8 if key_type == 'randint' else 32, value_size, cache_size, MN_num, threshold, span_size)
            ...
            YCSB_TEST = f"{env_cmd} && ./zipfian_test 1 0 40 0 2"
            ...
            print_GOOD(f"[FINISHED POINT] method={method} threshold={threshold} tpt={tpt}")
